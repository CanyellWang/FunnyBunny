...
object UserLocation {
  ...
  def main(args: Array[String]) {
    ...
    val rdd1 = sc.textFile("/Users/wangchangye/学习/大数据/code/day28_Spark/练习/bs_log").map(x => {
      val arr = x.split(",")
      val mb = (arr(0),arr(2))
      val flag = arr(3)
    ...
    val rdd3 = sc.textFile("/Users/wangchangye/学习/大数据/code/day28_Spark/练习/lac_info.txt").map(x => {
      val arr = x.split(",")
      val bs = arr(0)
      (bs, (arr(1), arr(2)))
    })

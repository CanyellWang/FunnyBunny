...
object GameKPI {
  ...
  def main(args: Array[String]) {
    ...
    //切分之后的数据
    val splitedLogs = sc.textFile("/Users/wangchangye/WorkSpace/GameLog.txt").map(_.split("\\|"))
    //过滤后并缓冲
    ...
    val dau = filteredLogs.filter(fields => FilterUtils.filterByTypes(fields, EventType.REGISTER, EventType.LOGIN))
      ...
      .distinct()
      .count()
    println("endTime--->%s",endTime)
    println("DNU日新增用户--->%s",dnu)
    println("DAU日活跃用户--->%s",dau)
    ...
    println("lastDayRegUser-->%s",lastDayRegUser.count())
    println("todayLoginUser-->%s",todayLoginUser.collect().length)

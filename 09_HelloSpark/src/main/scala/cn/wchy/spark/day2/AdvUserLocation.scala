...
object AdvUserLocation {
  ...
  def main(args: Array[String]) {
    ...
    val rdd0 = sc.textFile("/Users/wangchangye/学习/大数据/code/day28_Spark/练习/bs_log").map( line => {
    ...
    val rdd2 = sc.textFile("/Users/wangchangye/学习/大数据/code/day28_Spark/练习/lac_info.txt").map(line => {
      val f = line.split(",")
      //(基站ID， （经度，纬度）)
      (f(0), (f(1), f(2)))
    })

...
object IPLocation {
  ...
  def main(args: Array[String]) {
    ...
    val ipRulesRdd = sc.textFile("/Users/wangchangye/WorkSpace/ip.txt").map(line =>{
    ...
    println(ipRulesArrary.mkString)
    //广播规则
    val ipRulesBroadcast = sc.broadcast(ipRulesArrary)

    //加载要处理的数据
    val ipsRDD = sc.textFile("/Users/wangchangye/WorkSpace/access.log").map(line => {
      val fields = line.split("\\|")
      fields(1)
    })

...
public class BaseUserRecommender {
    ...
    public static void main(String[] args) throws Exception {
        //准备数据 电影评分数据
        File file = new File("/media/wangchangye/NewDisk/workspace/github/FunnyBunny/ml-latest-small/ratings.csv");
        //读取文件
        DataModel model = new FileDataModel(file);
        //使用欧几里得相似度算法
        UserSimilarity user = new EuclideanDistanceSimilarity(model);
        //计算最近邻域，邻居有两种算法，基于固定数量的邻居和基于相似度的邻居，这里使用基于固定数量的邻居
        NearestNUserNeighborhood neighbor = new NearestNUserNeighborhood(100, user, model);//选取100个邻居
        //构建推荐器
        Recommender r = new GenericUserBasedRecommender(model, neighbor, user);
        //为用户5推荐10个可能喜欢的电影
        List<RecommendedItem> rm = r.recommend(5, 10);
        //打印推荐结果
        System.out.println("使用基于用户的协同过滤算法");
        System.out.println("为用户5推荐10个电影");
        for(RecommendedItem m : rm){
            System.out.printf("(电影编号:%s,偏好度%f)", m.getItemID(), m.getValue());
            System.out.println();
        }
